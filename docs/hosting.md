...
1. Settings → Secrets → Actions:
   ...
   - `GSC_SERVICE_ACCOUNT_JSON` (optional; Google service account JSON for keyword collect)
...
1. Run migrations `20260903180000_seo_geo_snapshots.sql`, `20260911143000_seo_geo_one_snapshot_per_day.sql`, `20260911160000_seo_geo_collect_rpc.sql`, `20260918000000_seo_geo_gsc_queries.sql`, and `20260925120000_seo_geo_gsc_all_properties.sql` in the Supabase SQL editor.
2. Add GitHub secret `SEO_GEO_SUPABASE_SERVICE_ROLE_KEY` (nightly Action only).
3. For keywords, add GitHub secret `GSC_SERVICE_ACCOUNT_JSON` (Search Console service account JSON) and the same value as an Edge Function secret on `seo-geo-collect-gsc`. Grant that account on each Search Console property.
4. In `/seo-geo`, click **Refresh** on a property (or wait for 06:00 UTC). Keywords: `/seo-geo/:propertyId?tab=keywords`.
