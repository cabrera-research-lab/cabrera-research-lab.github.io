...
-- Fixes: "new row violates row-level security policy for table update_comments"

-- 1) Security-definer RPC (bypasses RLS; preferred path for the app)
...
-- 2) Direct-insert policies (fallback + ratings helper inserts)
...
-- Daily + weekly: any signed-in user may reply (cross-team Step 2 chat)
...
-- Monthly + quarterly: teammates may reply
