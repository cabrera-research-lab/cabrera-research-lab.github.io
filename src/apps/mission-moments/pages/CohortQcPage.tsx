...
import {
  ...
  runQc,
} from '@/apps/mission-moments/lib/qcApplet';
...
export function CohortQcPage() {
  ...
  useEffect(() => {
    if (!cohortId) {
      navigate(MISSION_MOMENTS_BASE, { replace: true });
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        navigate(missionMomentsPath(id), { replace: true });
      } else {
  ...
  return (
    <div className="b2b-qc">
      ...
      <div className="b2b-qc-detail-bar">
        <Link to={MISSION_MOMENTS_BASE} className="b2b-qc-back">
          ← All cohorts
        </Link>
        <div className="b2b-qc-detail-actions">
          {!session && isSupabaseConfigured && (
            <Link to={missionMomentsLoginPath()} className="b2b-qc-link">
              Sign in to save
            </Link>
          )}
          <button
